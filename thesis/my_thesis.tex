...
The results are organised by observational unit. Section~\ref{sec:network_evolution} examines the monthly graphs, using original-voyage summaries for aggregate trade volume and voyage counts. 
Section~\ref{sec:activity_sparsity} examines activity in the balanced node--month dataset, and Section~\ref{sec:chokepoint_criticality_measures} describes the chokepoint network measures. 
Section~\ref{sec:terminal_trade_patterns} examines terminal roles and counterparty measures in the terminal--month dataset derived from matched voyages. 

As described in Section~\ref{subsec:temporal_aggregation}, both datasets retain inactive months; summaries restricted to active observations are identified where they appear.

\section{Network evolution and connectivity}
\label{sec:network_evolution}

Table~\ref{tab:eda_monthly_summary} summarises the monthly numbers of active nodes, directed edges and distinct export voyages, as well as exported LNG volume and directed density. 
Active nodes and edges generally increase from 2021 to late 2023, reaching their respective maxima in December 2023, before declining during the second half of 2024. 
These changes describe participation in the observed network rather than changes in the set of potential nodes.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrr}
    ...
    Distinct export voyages & 70 & 144.0 & 237 \\
...
chokepoints, and directed connections in June 2021. This month has
98 active nodes and 138 edges, therefore it displays values close to the monthly
averages reported in Table~\ref{tab:eda_monthly_summary}. 
...
\begin{figure}[H]
    ...
    direction of observed network connections. Connections are drawn
    schematically between node coordinates and do not represent vessel
    trajectories.}
...
in the table. The increase through 2023 is consistent with the broader
expansion of global LNG trade described by \citet{zhou2026}; the comparison
provides context rather than an explanation for every monthly fluctuation.
As explained in Chapter 3, monthly export volume is calculated from the
original voyages, since summing flows over successive network edges would
count some shipments more than once.

\begin{figure}[H]
  \centering
  \includegraphics[width=.82\textwidth]{images/global_lng_export_volume_over_time.png}
  \caption{Monthly LNG export volume based on distinct observed export voyages (cubic metres).}
...
The monthly graph is not always weakly connected, although its largest weakly connected component contains at least 89.8\% of active nodes throughout the period. 
Thus, most active nodes belong to one component even when smaller components exist. 
The bridge and articulation measures examine a different aspect of connectivity: whether removing particular nodes or connections disrupts it.

The temporal quality-assurance indicator identifies months with at least two of three observed quantities---voyages, original-voyage volume and active edges---below 70\% of their respective 60-month medians. 
It flags several months in 2020 and early 2021, as well as November--December 2024. 
The flag identifies unusually low \emph{observed activity}, not missing data or a verified external shock. 
In particular, the late-2024 decline should not be assigned a substantive cause without separate evidence.

\section{Node activity and temporal sparsity}
\label{sec:activity_sparsity}

A node is active in month $t$ when it has positive observed throughput in that month. Of 10,980 potential node-month observations ($183\times60$), 5,792 (52.75\%) are active and 5,188 (47.25\%) inactive. The monthly inactive share ranges from 31.15\% to 60.66\%. This is sparsity in \emph{node activity}; it should not be conflated with directed graph density, which uses active nodes as its denominator.

Terminals account for 9,540 observations, of which 4,552 are active and 4,988 inactive (52.29\% inactive). The 24 chokepoints account for 1,440 observations, of which 1,240 are active and 200 inactive (13.89\% inactive). Thus, the aggregate inactivity pattern is predominantly a terminal phenomenon, although chokepoint activity is not constant. The different base rates also caution against assuming that the two node types share the same activity process.
...
Inactivity is substantively relevant to the interpretation of centrality: an inactive node may receive a zero in the balanced node-month dataset even though the criticality measure is chiefly meaningful when it participates in trade. A later activity component can therefore address participation separately from the magnitude of criticality conditional on activity. The descriptive shares support examining such a specification; they do not by themselves establish the best probability model or its predictors.

\section{Terminal roles and criticality}
\label{sec:terminal_trade_patterns}

Terminal roles are assigned from observed monthly direction of flow. Among the 9,540 terminal-months, 3,234 are importer months, 1,318 exporter months and 4,988 inactive months. There are no bidirectional terminal-months in the present data. The role is a monthly network observation, whereas infrastructure type is a terminal characteristic; their agreement should be evaluated empirically rather than assumed. The country summaries instead classify each country by its \emph{net} monthly trade position, so a net importing country can contain an exporting terminal and country counts cannot be interpreted as sums of terminal roles.

For active terminals, median throughput is 346,000 cubic metres, median voyage count is two, and the median number of distinct terminal counterparties is two. The 95th percentile of throughput is approximately 1.90 million cubic metres, compared with a maximum of 10.91 million. This skew motivates using transformed lagged throughput as a possible covariate and keeping operational scale conceptually distinct from network position.

\subsection{Centrality, concentration and dependence}
\label{subsec:terminal_criticality}

The role-specific PageRank uses the reversed terminal trade graph for exporters and the original graph for importers; in each case the directed edges are weighted by LNG volume. It has a median of 0.0142 among active terminal-months (95th percentile 0.0363). Its interpretation depends on the observed role, and it is computed on the active monthly terminal graph before the balanced panel assigns zero to inactive terminals.

The counterparty HHI describes how the terminal's relevant inflow or outflow is distributed among its counterparties. An HHI of one means that all role-relevant flow involves one terminal or one country. Among active terminal-months, the median terminal-level HHI is 0.556 and the median country-level HHI is 0.674; the respective 75th percentiles are both one. The country-level index aggregates counterparties by country and can consequently be higher than its terminal-level counterpart. The effective-counterparty quantities are their reciprocals and should be treated as alternative expressions of the same concentration measures, not additional independent outcomes.

The role-specific dependence measure has a median of 0.406 and a 75th percentile of 0.782 among active terminal-months. Its construction differs by role: for exporters, it is the export-flow-weighted share of each recipient terminal's imports supplied by that exporter; for importers, it is the terminal's share of its country's observed LNG receipts. These two interpretations must be reported explicitly when comparing the pooled role-specific measure. In particular, counterparty HHI describes concentration of a terminal's own trade, whereas dependence asks how much others rely on that terminal under the relevant role-specific definition.

\subsection{Structural position}
\label{subsec:terminal_structure}

The monthly undirected projection provides complementary binary indicators: whether a terminal is an articulation point, whether it is incident to a global bridge or to a local bridge that is not global, and whether it is adjacent to an articulation point. They reflect connectivity and exposure to fragile connections rather than trade volume. These indicators can be discussed alongside centrality and dependence, without treating them as interchangeable. For terminals, shortest-path betweenness in the directed monthly network is identically zero in the published node-month summary and consequently offers no variation for a terminal criticality outcome.
