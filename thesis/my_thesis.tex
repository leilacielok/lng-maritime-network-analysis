...
Of the 9,540 terminal--month observations, 3,234 are active importer
months, 1,318 are active exporter months, and 4,988 are inactive months.

Country-level roles were assessed over the full 2020--2024 period
using each country's net trade position. Qatar, the United States,
and Russia recorded the largest cumulative export volumes, while
China, France, and Japan recorded the largest cumulative import
volumes. A net role does not imply trade in only one direction:
Malaysia was a net importer despite recording exports, and the
United States was a net exporter despite recording imports.

Table~\ref{tab:eda_terminal_activity_by_role} compares throughput,
voyage counts, and numbers of terminal counterparties across active
importer and exporter months. Exporter observations have higher
medians and 95th percentiles for all three measures. For example,
median monthly throughput is approximately 0.505 million cubic
metres for exporters and 0.328 million cubic metres for importers.
...
The differences in voyage counts and counterparties shown in Table~\ref{tab:eda_terminal_activity_by_role}
also indicate that monthly terminal activity varies in both operational scale and the number of trading connections.


\subsection{Centrality, concentration and dependence}
\label{subsec:terminal_criticality_eda}

The role-specific PageRank uses the reversed terminal trade graph for exporters and the original graph for importers; in each case the directed edges are weighted by LNG volume. It has a median of 0.0142 among active terminal-months (95th percentile 0.0363). 
Its interpretation depends on the observed role, and it is computed on the active monthly terminal graph before the balanced panel assigns zero to inactive terminals.

The counterparty HHI describes how the terminal's relevant inflow or outflow is distributed among its counterparties. 
An HHI of one means that all role-relevant flow involves one terminal or one country. Among active terminal-months, the median terminal-level HHI is 0.556 and the median country-level HHI is 0.674; the respective 75th percentiles are both one. 
The country-level index aggregates counterparties by country and can consequently be higher than its terminal-level counterpart. The effective-counterparty quantities are their reciprocals and should be treated as alternative expressions of the same concentration measures, not additional independent outcomes.

The role-specific dependence measure has a median of 0.406 and a 75th percentile of 0.782 among active terminal-months. 
Its construction differs by role: for exporters, it is the export-flow-weighted share of each recipient terminal's imports supplied by that exporter; for importers, it is the terminal's share of its country's observed LNG receipts. 
These two interpretations must be reported explicitly when comparing the pooled role-specific measure. 
In particular, counterparty HHI describes concentration of a terminal's own trade, whereas dependence asks how much others rely on that terminal under the relevant role-specific definition.

\subsection{Structural position}
\label{subsec:terminal_structure}

The monthly undirected projection provides complementary binary indicators: 
whether a terminal is an articulation point, whether it is incident to a global bridge or to a local bridge that is not global, and whether it is adjacent to an articulation point. 
