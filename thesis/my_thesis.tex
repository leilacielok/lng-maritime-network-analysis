...
The numbers of active nodes and directed edges generally increase from 2021 to late 2023, both reaching their observed maxima in December 2023, before declining during the second half of 2024. Table~\ref{tab:eda_monthly_summary} reports their monthly ranges and averages. These changes describe participation in the observed network rather than changes in the set of potential nodes.

Distinct export voyages and exported LNG volume also vary substantially across months (Table~\ref{tab:eda_monthly_summary}). Figure~\ref{fig:eda_export_volume} shows the volume series. As described in Chapter 3, this aggregate comes from the original voyages rather than the sum of flows on successive network edges.
...
Directed density is calculated among active nodes, so its changing denominator matters: a month with fewer edges can still have higher density if fewer nodes are active. The monthly graph is not always weakly connected, although its largest weakly connected component contains at least 89.8\% of active nodes throughout the period. Thus, most active nodes belong to one component even when smaller components exist. The bridge and articulation measures examine a different aspect of connectivity: whether removing particular nodes or connections disrupts it.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrr}
    ...
    \hline
  \end{tabular}
  \begin{flushleft}\footnotesize Sources: \texttt{monthly\_network\_statistics.csv} and \texttt{monthly\_network\_structure.csv}; rounded descriptive values.\end{flushleft}
\end{table}

The temporal quality-assurance indicator identifies months with at least two of three observed quantities---voyages, original-voyage volume and active edges---below 70\% of their respective 60-month medians. It flags several months in 2020 and early 2021, as well as November--December 2024. The flag identifies unusually low \emph{observed activity}, not missing data or a verified external shock. In particular, the late-2024 decline should not be assigned a substantive cause without separate evidence.
